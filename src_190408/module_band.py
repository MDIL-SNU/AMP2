	...
	if metal != 0:
		...
		for i in range(3) :
			...
			if not total_VBM_kpt[i] == total_CBM_kpt[i]:
				direct = 1
		if direct == 0 :
			gap_log.write('Band gap: '+'{:10.3f}'.format(gap)+' eV (Direct)\n\n')
		else :
			gap_log.write('Band gap: '+'{:10.3f}'.format(gap)+' eV (Indirect)\n\n')
		gap_simple.write('     '+'{:10.3f}'.format(gap))
		gap_log.write('VBM: '+'  '.join(total_VBM_kpt)+'   : '+'{:10.3f}'.format(total_VBM)+' eV\n')
		gap_log.write('CBM: '+'  '.join(total_CBM_kpt)+'   : '+'{:10.3f}'.format(total_CBM)+' eV\n')
