...
#			if idx_GGA[k] < 74 and idx_GGA[k] > 66:
#				print [Band[x][k][i] for x in [25,26,27,28,29,30]]
#				np.savetxt('Mat1_{:d}_{:03d}.dat'.format(i+1,idx_GGA[k]),M[7:18,7:17],fmt='%4.2f')
#				np.savetxt('Mat2_{:d}_{:03d}.dat'.format(i+1,idx_GGA[k]),M2[7:18,7:17],fmt='%4.2f')
			...
			for n1 in range(min_band,max_band):
				for n2 in range(min_band,max_band):
					if R[n1].sum() == 1 and not flag[n2] == 0:
						...
						M_remain[:,n2] = 0.0
			while 1 :
				ind = np.unravel_index(np.argmax(M_remain),M_remain.shape)
				if M_remain[ind] < min_overlap_cut:
					break
				else:
					R[ind] = 1
					M_remain[ind[0]] = 0.0
...
plot_band_new_structure(spin,Band_reorder,eVBM,gga_path+'/xtic_all.dat',gga_path+'/xlabel.dat',[5,5],gga_path)
