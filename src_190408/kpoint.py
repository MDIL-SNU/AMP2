...
#		print("Too many k-point mesh is required!")
	...
	if rerun == 0:
		...
		if out == 2:  # electronic step is not converged. (algo = normal)
			make_amp2_log(dir+'/kptest','Electronic step is not converged. ALGO is already normal.')
			print 0
			sys.exit()
		elif out == 1:  # elctronic step is not converged. (algo = fast) Algo changes to normal and rerun.
			make_amp2_log(dir+'/kptest','Electronic step is not converged. ALGO changes to Normal.')
			out = run_vasp(now_path,nproc,vasprun)
			if out == 1:  # error in vasp calculation
				print 0
				sys.exit()
